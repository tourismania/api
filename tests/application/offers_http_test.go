...
func newOffersTestRouter(jwtSvc *auth.Service, users custommw.UserFinder, createUC createoffer.UseCase, listUC getoffers.UseCase) http.Handler {
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Use(custommw.CurrentUserMiddleware(users))

		agentOrAdmin := custommw.RequireRole(enum.RoleAgent, enum.RoleSuperAdmin)
		api.Route("/offers", func(offers chi.Router) {
			offers.With(agentOrAdmin).Post("/", createH.Handle)
			offers.Get("/", listH.Handle)
		})
	})
	return r
}
...
func TestOffersHTTP_ListOffers_PlainUser_ForcesPublishedFilter(t *testing.T) {
	jwtSvc := newTestJWTService(t)
	userUUID := uuid.New()
	token, err := jwtSvc.Issue(userUUID)
	require.NoError(t, err)
	...
	assert.Equal(t, 1, listUC.gotQuery.CurrentUserID)
	assert.Equal(t, []enum.Role{enum.RoleUser}, listUC.gotQuery.CurrentRoles)
}

func TestOffersHTTP_ListOffers_NoToken_Returns401(t *testing.T) {
	jwtSvc := newTestJWTService(t)
	r := newOffersTestRouter(jwtSvc, stubUserFinder{}, &stubCreateOfferUseCase{}, &stubGetOffersUseCase{})
