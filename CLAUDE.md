...
- **Code review → сохранять результат.** После каждого code review создавать файл `docs/code_reviews/{branch_name}_v{N}.md`, где `branch_name` — слэши заменяются на дефисы (например `feature/3` → `feature-3`), `N` — порядковый номер ревью для этой ветки (начинается с 1). Файл содержит: дату, ветку, scope диффа, метод ревью, все находки с severity, failure scenario и summary-таблицу.
