...
docs/
  swagger/            # OpenAPI (генерируется через swag, НЕ РЕДАКТИРОВАТЬ вручную)
  curls/              # curl-сценарии для Postman/Insomnia (вручную, см. docs/curls/README.md)
  test_cases/         # чек-лист сценариев по доменам (вручную)
  issues/             # ТЗ по issue (вручную)
  tech_debt/          # реестр техдолга (вручную)
  cli/                # доп. документация по CLI-командам (вручную)
tests/
...
- Для работы с техническим заданием подключай агента - engineering-technical-writer
  - В процессе создания ТЗ когда ссылаешься на какие-нибудь пункты, не используй синтаксис `#1`, используй `№1`. Чтобы при формировании задачи в Github не подставлялись ссылки на задачи
...
- Единый паттерн именования команд: `<ресурс> <действие>` (группировка по существительному, например `user create`, `agency create`, `agency deactivate`) — не `<действие>-<ресурс>` (устаревший стиль, был `create-user`).

### General

- Все публичные функции и типы — с Go-doc комментариями.
- Все комментарии в коде (Go-doc и inline) — на русском языке.
...
| `docs/curls`   | Вручную — при изменении контракта существующего endpoint (новое/переименованное поле, новый код ошибки) или добавлении нового endpoint. Актуальный HTTP-контракт всегда можно получить заново импортом `docs/swagger/swagger.json` в Postman/Insomnia (см. `docs/curls/README.md`) — вручную поддерживаются только бизнес-сценарии, не форма ответа |
| `docs/test_cases/test_cases.md` | Вручную — при добавлении/изменении сценария домена или endpoint'а (новый пункт чек-листа, ссылающийся на соответствующий файл в `docs/curls`) |
