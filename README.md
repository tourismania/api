...
**Видимость offer (read-side):** `GET /api/v1/offers*` — публичные эндпоинты, JWT не обязателен. Опубликованный (`status=published`) offer виден любому, включая неавторизованных пользователей. Если запрос аутентифицирован и пользователь — сотрудник агентства (`ROLE_AGENT` или `ROLE_SUPER_ADMIN`), он дополнительно видит offer **своего** агентства в любом статусе (черновики включительно); фильтр `agency_id` в этом случае всегда принудительно выставляется в собственное агентство пользователя — 1 пользователь = 1 агентство, кросс-агентского доступа нет даже у `ROLE_SUPER_ADMIN`. `ROLE_USER`-клиенты видят только `published`, независимо от своего `agency_id`.
