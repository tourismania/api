...
- `GET /api/v1/offers` и `GET /api/v1/offers/{uuid}` требуют JWT (`custommw.JWT` + `CurrentUserMiddleware`). `GET /api/v1/public/offers/{uuid}` — отдельный, полностью анонимный обработчик без auth-мидлварей вообще.
