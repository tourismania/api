...
| GET   | /api/v1/offers   | public (auth опционален) | Список offer (пагинация + фильтры); видимость — см. ниже |
| GET   | /api/v1/offers/{uuid} | public (auth опционален) | Получить один offer; видимость — см. ниже |
...
**Статусы offer:** `draft` (черновик, редактируется) → `ready` (заполнен и сохранён, но агент ещё не решил публиковать) → `published` (виден всем). `draft` и `ready` видны только сотрудникам своего агентства; переход между статусами делается через `PATCH .../offers/{uuid}` (`status`).

**Видимость offer (read-side):** `GET /api/v1/offers*` — публичные эндпоинты, JWT не обязателен. Опубликованный (`status=published`) offer виден любому, включая неавторизованных пользователей. Если запрос аутентифицирован и пользователь — сотрудник агентства (`ROLE_AGENT` или `ROLE_SUPER_ADMIN`), он дополнительно видит offer **своего** агентства в любом статусе (`draft`/`ready` включительно); фильтр `agency_id` в этом случае всегда принудительно выставляется в собственное агентство пользователя — 1 пользователь = 1 агентство, кросс-агентского доступа нет даже у `ROLE_SUPER_ADMIN`. `ROLE_USER`-клиенты видят только `published`, независимо от своего `agency_id`.

**Владение по агентству (write-side):** создавать/изменять/удалять offer может только `ROLE_AGENT` или `ROLE_SUPER_ADMIN`, принадлежащий **тому же** агентству, что и offer (`offer.agency_id == user.agency_id`); чужое агентство → `403`. Особого режима для `ROLE_SUPER_ADMIN` нет — 1 пользователь = 1 агентство касается всех ролей одинаково.
...
| Роль / гость       | Offers: создание/изменение/удаление | Offers: чтение |
|--------------------|--------------------------------------|-----------------|
| `ROLE_SUPER_ADMIN` | Только offer своего агентства (`offer.agency_id == user.agency_id`); чужое агентство → `403` — особого режима у супер-админа нет, 1 пользователь = 1 агентство касается всех ролей | Любой offer своего агентства (черновик и опубликованный) + опубликованные offer любых агентств |
...
| Неавторизованный   | Недоступно (`401` на write-эндпоинты) | Только `status = published`, любого агентства — GET-эндпоинты offer публичные |

- **1 пользователь = 1 агентство касается всех ролей одинаково — кросс-агентского доступа нет даже у `ROLE_SUPER_ADMIN`.** Владение по агентству жёстко привязано к `AgencyID` пользователя, не к `CreatedBy` (тот — только аудит).
...
- `GET /api/v1/offers` и `GET /api/v1/offers/{uuid}` не требуют JWT: `OptionalJWT`/`OptionalCurrentUser` резолвят принципала, если токен присутствует, иначе запрос обрабатывается как гостевой (видны только `published`).
- Guard `RequireRole(...)` (мидлварь) проверяет принадлежность к роли на границе HTTP для write-эндпоинтов; проверку владения агентством выполняет доменный `OfferManager`.
