...
# migrate — применение/откат SQL-миграций; устанавливается здесь, чтобы
#           runner мог скопировать бинарник, не завися от stage dev.
...
COPY --from=builder --chown=app:app /go/bin/migrate  /app/migrate
